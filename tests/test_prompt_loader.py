...
from hippos.llm.prompt_loader import load_prompt_text, render_prompt, resolve_prompt_text
...
def test_project_prompt_override(tmp_path: Path):
    prompt_dir = tmp_path / ".hippos" / "prompts"
...
def test_env_prompt_override(monkeypatch, tmp_path: Path):
    ...
    monkeypatch.setenv("HIPPOS_PROMPTS_DIR", str(prompt_dir))
    text = load_prompt_text("phase-1-system.md")
