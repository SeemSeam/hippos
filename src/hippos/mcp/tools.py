"""Navigation-oriented MCP tool implementations for hippos."""
...
from ..paths import index_file_for_read, state_dir_for_read
from ..utils import read_json


def _load_index(hippos_dir: Path) -> dict[str, Any] | None:
    index_path = index_file_for_read(hippos_dir)
...
def _repomap_ranked_context(
    ...
    *,
    ...
    from ..tools.repomap_adapter import HipposRepoMap
    ...
    repomap = HipposRepoMap(root, index_files=set(all_files))
...
def navigate_tool(
    ...
    hippos_dir: Path | None = None,
) -> dict[str, Any]:
    """Find relevant files and snippets for a task."""
    del snapshot_ref
    hippos_dir = hippos_dir or state_dir_for_read(Path.cwd())
    root = hippos_dir.parent.resolve()
    index = _load_index(hippos_dir)
    if index is None:
        return {"error": "Index not found. Run 'hippos index' first."}
