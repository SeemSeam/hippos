...
def load_hippos_rules(project_root: str | Path) -> ArchitectureRules:
    legacy = load_architecture_rules(project_root, tool_name="hippo")
    current = load_architecture_rules(project_root, tool_name="hippos")
    return _merge_rules(legacy, current)


# Migration alias for callers moving from the old helper name.
load_hippo_rules = load_hippos_rules
