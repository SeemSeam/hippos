...
from ..architecture_rules import load_hippos_rules
...
def _collect_source_files(target: Path) -> list[Path]:
    ...
    rules = load_hippos_rules(target)
    files = []
...
def run_sig_extract(
    ...
    from ..constants import HIPPOS_DIR, LEGACY_HIPPOS_DIR, QUERIES_DIR

    local_queries = target / HIPPOS_DIR / QUERIES_DIR
    ...
    legacy_queries = target / LEGACY_HIPPOS_DIR / QUERIES_DIR
    if legacy_queries.is_dir():
        return _extract_signatures_with_queries(
            target=target,
            output_dir=output_dir,
            queries_dir=legacy_queries,
        )
    ...
    raise FileNotFoundError(
        "Queries directory not found. Run 'hippos init' first."
    )
