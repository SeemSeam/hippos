...
_STRUCTURE_MAP_PATH = Path(".hippos/structure-prompt.md")


def _load_structure_map() -> str:
    if not _STRUCTURE_MAP_PATH.exists():
        return "No architecture map found. Please run 'hippos index' first."
...
CONTEXT:
Project Architecture (from .hippos/structure-prompt.md):
```markdown
{structure_map}
```
...
class Reviewer:
    ...
    def __init__(self, config: HipposConfig | None = None):
        self.config = config or HipposConfig()
