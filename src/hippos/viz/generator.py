...
from ..constants import INDEX_FILE, TREE_FILE, VIZ_FILE
from ..paths import index_file_for_read
...
def generate_viz_html(
    ...
    Args:
        output_dir: Directory containing hippos data files
    ...
    # Load data sources
    index_path = index_file_for_read(output_dir)
    index = load_json_file(index_path)
    tree = load_json_file(output_dir / TREE_FILE)

    if not index:
        raise FileNotFoundError(f"{INDEX_FILE} not found in {output_dir}")
