...
def resolve_env_value(value: object) -> str:
    resolved = resolve_env_refs(value)
    return str(resolved or "")

HIPPOS_LLM_CONFIG_NAME = "config.yaml"
LEGACY_HIPPOS_USER_CONFIG_DIR = ".hippocampus"
LEGACY_HIPPOS_USER_CONFIG_ENV = "HIPPOCAMPUS_USER_CONFIG_DIR"
LEGACY_HIPPOS_LLM_CONFIG_ENV = "HIPPOCAMPUS_LLM_CONFIG"
...
def hippos_user_config_dir() -> Path:
    override = str(os.environ.get("HIPPOS_USER_CONFIG_DIR", "") or "").strip()
    if override:
        return Path(override).expanduser().resolve()
    legacy_override = str(os.environ.get(LEGACY_HIPPOS_USER_CONFIG_ENV, "") or "").strip()
    if legacy_override:
        return Path(legacy_override).expanduser().resolve()
    legacy_dir = (Path.home() / LEGACY_HIPPOS_USER_CONFIG_DIR).resolve()
...
def resolve_user_llm_config_file(path: str | Path | None = None) -> Path:
    ...
    explicit = str(os.environ.get("HIPPOS_LLM_CONFIG", "") or "").strip()
    if explicit:
        return Path(explicit).expanduser().resolve()
    legacy_explicit = str(os.environ.get(LEGACY_HIPPOS_LLM_CONFIG_ENV, "") or "").strip()
    if legacy_explicit:
        return Path(legacy_explicit).expanduser().resolve()
    return hippos_user_config_dir() / HIPPOS_LLM_CONFIG_NAME
...
# Migration aliases for a few versions.
HIPPOCAMPUS_LLM_CONFIG_NAME = HIPPOS_LLM_CONFIG_NAME
hippocampus_user_config_dir = hippos_user_config_dir
