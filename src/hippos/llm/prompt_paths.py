...
def packaged_prompt_resource(name: str):
    return files("hippos").joinpath("resources", "prompts", name)


def project_prompts_dir(project_root: str | Path | None) -> Path | None:
    if project_root is None:
        return None
    return Path(project_root).resolve() / ".hippos" / "prompts"
...
def legacy_project_prompts_dir(project_root: str | Path | None) -> Path | None:
    ...
    return root / ".hippocampus" / "prompts"


def override_prompts_dir() -> Path | None:
    override = str(os.environ.get("HIPPOS_PROMPTS_DIR", "") or "").strip()
    if override:
        return Path(override).expanduser().resolve()
    legacy_override = str(os.environ.get("HIPPOCAMPUS_PROMPTS_DIR", "") or "").strip()
    if legacy_override:
