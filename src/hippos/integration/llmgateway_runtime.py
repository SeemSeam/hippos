...
try:
    from llmgateway.config import (
        load_user_config as _load_gateway_user_config,
        resolve_env_refs as _resolve_env_refs,
        runtime_spec_from_dict as _runtime_spec_from_dict,
    )
except ModuleNotFoundError:  # pragma: no cover - dependency-light envs
    _load_gateway_user_config = None
    _resolve_env_refs = None
    _runtime_spec_from_dict = None
...
def resolve_env_refs(value):
    if _resolve_env_refs is not None:
...
def load_gateway_user_config(path: str | Path) -> dict[str, Any]:
    if _load_gateway_user_config is not None:
        return _load_gateway_user_config(path)
    ...
    try:
        ...
        return {}
    return resolve_env_refs(loaded) if isinstance(loaded, dict) else {}
...
def runtime_profile_from_gateway_raw(raw: dict[str, Any]) -> dict[str, Any]:
    if _runtime_spec_from_dict is not None:
        runtime = _runtime_spec_from_dict(raw)
        task_models = {
    ...
    provider = resolved.get("provider", {}) if isinstance(resolved.get("provider"), dict) else {}
    settings = resolved.get("settings", {}) if isinstance(resolved.get("settings"), dict) else {}
    tasks = resolved.get("tasks", {}) if isinstance(resolved.get("tasks"), dict) else {}
    task_models = {
        str(name): str(task.get("model", "") or "").strip()
        for name, task in tasks.items()
        if isinstance(task, dict) and str(task.get("model", "") or "").strip()
    }
    return {
        "provider_type": str(provider.get("provider_type", "") or "").strip(),
        "api_style": str(provider.get("api_style", "") or "").strip(),
        "base_url": str(provider.get("base_url", provider.get("api_base", "")) or "").strip(),
        "api_key": str(provider.get("api_key", "") or "").strip(),
        "extra_headers": dict(provider.get("headers", {}) or {}),
        "model_map": dict(provider.get("model_map", {}) or {}),
        "fallback_model": str(settings.get("fallback_model", "") or "").strip(),
        "strong_model": str(settings.get("strong_model", "") or "").strip(),
        "weak_model": str(settings.get("weak_model", "") or "").strip(),
        "strong_reasoning_effort": str(settings.get("strong_reasoning_effort", "") or "").strip().lower(),
        "weak_reasoning_effort": str(settings.get("weak_reasoning_effort", "") or "").strip().lower(),
        "max_concurrent": max(1, int(settings.get("max_concurrent", 12) or 12)),
        "retry_max": max(0, int(settings.get("retry_max", 3) or 3)),
        "timeout": float(settings.get("timeout", 90) or 90),
        "transport_retries": max(1, int(settings.get("transport_retries", 3) or 3)),
        "task_models": task_models,
    }
...
def describe_user_gateway_runtime_issue(path: str | Path | None = None) -> str | None:
    ...
    try:
        runtime = _runtime_spec_from_dict(resolved) if _runtime_spec_from_dict is not None else None
    except Exception as exc:
    ...
    if runtime is not None:
        base_url = str(runtime.provider.base_url or "").strip()
        api_key = str(runtime.provider.api_key or "").strip()
        models = (runtime.strong_model, runtime.weak_model, runtime.fallback_model)
    else:
        profile = runtime_profile_from_gateway_raw(resolved)
        base_url = str(profile.get("base_url", "") or "").strip()
        api_key = str(profile.get("api_key", "") or "").strip()
        models = (
            profile.get("strong_model", ""),
            profile.get("weak_model", ""),
