...
def build_repomap_command():
    ...
    def repomap(ctx, target, target_option, files, limit):
        ...
        from ..tools.repomap_adapter import HipposRepoMap, check_repomap_available
        ...
        try:
            repomap_obj = HipposRepoMap(root=tgt, map_tokens=2048, verbose=ctx.obj["verbose"])
...
def build_index_command():
    ...
    def index(ctx, target, target_option, phase_num, no_llm):
        """Generate unified index -> hippos-index.json."""
        target = resolve_target_value(target, target_option)
        tgt = Path(target).resolve()
        out = tgt / HIPPOS_DIR
        out.mkdir(parents=True, exist_ok=True)
