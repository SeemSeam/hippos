...
def _resolve_paths(config_path, output_dir, target):
    ...
    if config_path:
        ...
        cfg_path = Path(HIPPOS_DIR) / CONFIG_FILE
...
def register_project_bootstrap_commands(cli) -> dict[str, object]:
    ...
    def init(ctx, target, target_option):
        """Initialize .hippos/ directory with default config."""
        target = resolve_target_value(target, target_option)
        target = Path(target).resolve()
        hippos_dir = target / HIPPOS_DIR
        hippos_dir.mkdir(parents=True, exist_ok=True)

        config_file = hippos_dir / CONFIG_FILE
        ...
        queries_dst = hippos_dir / QUERIES_DIR
        ...
        if copied:
            ...
            click.echo("Warning: packaged query files not found, skipping .scm copy.")

        click.echo(f"Initialized {hippos_dir}")
