...
def navigate(
    ...
    Args:
        ...
        hippos_dir: Path to .hippos directory.
    """
    try:
        return _navigate_inner(
            query, focus_files, conversation_files, budget_tokens, hippos_dir
        )
...
def _navigate_inner(
    ...
    budget_tokens: int,
    hippos_dir: Path | None,
) -> NavigateResult:
    """Core navigation logic (may raise)."""
    if hippos_dir is None:
        hippos_dir = state_dir_for_read(Path.cwd())
